...
export function Sidebar({ course, completedLessons = [] }: SidebarProps) {
                  ...
                  <div className="flex flex-col space-y-1">
                    {module.lessons?.map((lesson, lessonIndex) => {
                      const isActive =
                        pathname ===
                        `/dashboard/courses/${course._id}/lessons/${lesson._id}`;
                      // returns true or false if the lesson is complete (.some method)
  ...
  return (
    <>
      {/* Thin Mobile Sidebar */}
      <aside
        className="fixed inset-y-0 left-0 z-50 flex flex-col items-center w-[60px] border-r 
      bg-background lg:hidden py-4 gap-y-4"
      >
